...
describe('UrlTable', () => {
  it('shows empty state when there are no URLs', () => {
    render(<UrlTable urls={[]} onDelete={vi.fn()} page={0} totalPages={0} onPageChange={vi.fn()} />);
    expect(screen.getByText(/no shortened urls yet/i)).toBeInTheDocument();
  });

  it('renders a row for each URL', () => {
    render(<UrlTable urls={mockUrls} onDelete={vi.fn()} page={0} totalPages={1} onPageChange={vi.fn()} />);
  ...
  it('calls onDelete with the alias when delete is clicked', () => {
    ...
    render(<UrlTable urls={mockUrls} onDelete={onDelete} page={0} totalPages={1} onPageChange={vi.fn()} />);
